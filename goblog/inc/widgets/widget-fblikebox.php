...
class bpxl_facebook_widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			...
			'(GoBlog) Facebook Like Box',		// Name
			array(
				'classname'		=>	'bpxl_facebook_widget',
				'description'	=>	__('A widget that displays a facebook like box from your facebook page.', 'bloompixel')
			)
		);
	} // end constructor
	...
	public function widget( $args, $instance ) {
		...
		if ( $title )
            ...
            if ( $this->color_scheme == 'dark' ) { $bpxl_fb_widget_bg = ' style="background:#141823"'; } else { $bpxl_fb_widget_bg = ''; }
		/* Like Box */
		?>
			<div class="fb-like-box"<?php echo $bpxl_fb_widget_bg; ?>
				data-href="<?php echo esc_url( $this->facebook_username ); ?>"
	...
	function form( $instance ) {
		...
		$defaults = array(
			'title' => '',
			'color_scheme' => 'light',
			'page_name' => 'http://www.facebook.com/bloompixel',
			'width' => '328',
			'show_faces' => '',
			'show_stream' => '',
			'show_header' => ''
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'page_name' ); ?>"><?php _e('Page URL (http://www.facebook.com/bloompixel)', 'bloompixel') ?></label>
			<input type="text" class="widefat" id="<?php echo $this->get_field_id( 'page_name' ); ?>" name="<?php echo $this->get_field_name( 'page_name' ); ?>" value="<?php echo esc_url( $instance['page_name'] ); ?>" />
		</p>
		...
		<!-- Color Scheme: Select -->
		<p>
			<label for="<?php echo $this->get_field_id( 'color_scheme' ); ?>"><?php _e('Color Scheme', 'bloompixel') ?></label>
		...
		<!-- Show Faces: Checkbox -->
		<p>
			<label for="<?php echo $this->get_field_id( 'show_faces' ); ?>"><?php _e('Show Faces', 'bloompixel') ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'show_stream' ); ?>"><?php _e('Show Stream', 'bloompixel') ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'show_header' ); ?>"><?php _e('Show Header', 'bloompixel') ?></label>
